...
public struct iPhoneNumberField: UIViewRepresentable {
  ...
  public func updateUIView(_ uiView: PhoneNumberTextField, context: UIViewRepresentableContext<Self>) {
    DispatchQueue.main.async {
      ...
      uiView.partialFormatter.defaultRegion = defaultRegion ?? PhoneNumberUtility.defaultRegionCode()
      if placeholder != nil {
        uiView.placeholder = placeholder
      } else {
  ...
  public class Coordinator: NSObject, UITextFieldDelegate {
    internal init(
      ...
      formatted: Bool,
      onBeginEditing: @escaping (PhoneNumberTextField) -> Void = { (view: PhoneNumberTextField) in },
      onEditingChange: @escaping (PhoneNumberTextField) -> Void = { (view: PhoneNumberTextField) in },
      onPhoneNumberChange: @escaping (PhoneNumber?) -> Void = { (view: PhoneNumber?) in },
      onEndEditing: @escaping (PhoneNumberTextField) -> Void = { (view: PhoneNumberTextField) in },
      onClear: @escaping (PhoneNumberTextField) -> Void = { (view: PhoneNumberTextField) in },
      onReturn: @escaping (PhoneNumberTextField) -> Void = { (view: PhoneNumberTextField) in }
    ) {
      self.text = text
